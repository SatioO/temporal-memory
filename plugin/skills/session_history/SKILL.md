...
Use the Bash tool to run: `curl -s -H "Authorization: Bearer ${AGENTMEMORY_SECRET:-}" "http://${AGENTMEMORY_URL:-localhost:3111}/graphmind/sessions" 2>/dev/null || echo '{"sessions":[]}'`
